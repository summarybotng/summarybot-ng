...
class GoogleAdminGroupsRepository:
    ...
    async def get_admin_groups(self, guild_id: str) -> List[str]:
        ...
        """
        query = """
        SELECT group_email FROM guild_google_admin_groups
        WHERE guild_id = ?
        ...
        try:
            ...
            return [row["group_email"] for row in rows]
    ...
    async def add_admin_group(
        ...
        Args:
            guild_id: Discord guild ID
            group_email: Google Workspace group email address
        ...
        existing = await self._get_mapping(guild_id, group_email)
        if existing:
            logger.info(
                f"Admin group {group_email} already exists for guild {guild_id}"
            )
            return False
        ...
        query = """
        INSERT INTO guild_google_admin_groups (id, guild_id, group_email, created_by, created_at)
        VALUES (?, ?, ?, ?, ?)
        """
        ...
        try:
            await self.connection.execute(
                query,
                (entry_id, guild_id, group_email, created_by, created_at),
            )
            logger.info(
                f"Added admin group {group_email} for guild {guild_id} by {created_by}"
            )
            return True
        except Exception as e:
            logger.error(
                f"Failed to add admin group {group_email} for guild {guild_id}: {e}"
            )
            raise

    async def remove_admin_group(self, guild_id: str, group_email: str) -> bool:
        """Remove an admin group email from a guild.

        Args:
            guild_id: Discord guild ID
            group_email: Google Workspace group email address to remove
        ...
        WHERE guild_id = ? AND group_email = ?
        """

        try:
            cursor = await self.connection.execute(query, (guild_id, group_email))
            deleted = cursor.rowcount > 0
            if deleted:
                logger.info(f"Removed admin group {group_email} from guild {guild_id}")
            else:
                logger.info(
                    f"Admin group {group_email} not found for guild {guild_id}"
                )
            return deleted
        except Exception as e:
            logger.error(
                f"Failed to remove admin group {group_email} from guild {guild_id}: {e}"
            )
            raise
    ...
    async def get_all_mappings(self) -> List[Dict[str, Any]]:
        ...
        Returns:
            List of mapping dictionaries containing id, guild_id, group_email,
            created_by, and created_at fields
        """
        query = """
        SELECT id, guild_id, group_email, created_by, created_at
    ...
    async def get_guilds_for_group(self, group_email: str) -> List[str]:
        ...
        try:
            rows = await self.connection.fetch_all(query, (group_email,))
            return [row["guild_id"] for row in rows]
        except Exception as e:
            logger.error(f"Failed to get guilds for group {group_email}: {e}")
            raise

    async def _get_mapping(
        self, guild_id: str, group_email: str
        ...
        try:
            return await self.connection.fetch_one(query, (guild_id, group_email))
        except Exception as e:
            logger.error(
                f"Failed to get mapping for guild {guild_id}, group {group_email}: {e}"
            )
            raise
